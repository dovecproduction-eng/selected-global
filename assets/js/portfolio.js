...
import { supabase, ALL_LISTINGS_URL, creatorContact, PUBLIC_PROPERTY_COLS } from './config.js?v=149';
import { t, applyI18n, getLang } from './i18n.js?v=149';
import {
  ICON, fmtPrice, esc, pickTitle, slugify, brandedCover, orderedPhotos,
...
function render() {
  ...
  // Kapağa tıkla → o dairenin fotoğraflarını tam ekran galeride aç
  grid.querySelectorAll('.pcard-cover[data-lb]').forEach((el) => {
    el.addEventListener('click', () => {
      const row = items.find((x) => x.id === el.dataset.lb);
      if (row) openLightbox(row.fotograflar, row.kapak_index || 0);
    });
  });
...
async function load() {
  ...
  portfolio = p;
  // İletişim = portföyü hazırlayan seçilen kişi (isim + telefon)
  ...
  if (ids.length) {
    ...
    items.forEach((r) => { r.fotograflar = orderedPhotos(r.fotograflar); });
  }
