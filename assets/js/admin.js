...
import { supabase, REGION_GROUPS, KONUT_TIPLERI, ODA_TIPLERI, PROJELER, STORAGE_BUCKET, CURRENCY, BRAND, ALL_LISTINGS_URL, nameFromEmail, CREATORS, creatorContact, SUPER_ADMIN_EMAIL } from './config.js?v=142';
import { ICON, esc, pickTitle, pickDesc, coverUrl, fmtPrice, toast, brandedCover, downloadPropertyPhotos, downloadReel, slugify, regionDistrict, regionDisplay, logoMark } from './ui.js?v=142';
...
const TITLE_TEMPLATES = [
  ...
  'Geniş ve aydınlık {unit} — {proje} projesinde',
  '{proje} projesinde ferah ve aydınlık {unit}',
  '{proje} projesinde geniş ferah {unit}',
  'Aydınlık bir yaşam {proje} projesinde: {unit}',
  '{proje} projesinde zarif {unit}',
  '{proje} projesinde estetik ve fonksiyonel {unit}',
  'Çağdaş bir yaşam {proje} projesinde: {unit}',
  '{proje} projesinde çağdaş {unit}',
  '{proje} projesinde nezih bir yaşam: {unit}',
  'Nezih bir çevrede {unit} — {proje} projesinde',
  '{proje} projesinde davetkâr {unit}',
  '{proje} projesinde akıllı planlı {unit}',
  'Kullanışlı ve fonksiyonel {unit}, {proje} projesinde',
  '{proje} projesinde gözde {unit}',
  'Şehrin yükselen değeri {proje}: {unit}',
  '{proje} projesinde geleceğin değeri: {unit}',
  'Değer kazanan {proje} projesinde {unit}',
  '{proje} projesinde kazançlı bir yatırım: {unit}',
  'Avantajlı konumuyla {proje} projesinde {unit}',
  '{proje} projesinde cazip bir fırsat: {unit}',
  '{proje} projesinde benzersiz {unit}',
  'Benzersiz bir yaşam {proje} projesinde: {unit}',
  '{proje} projesinde gözalıcı {unit}',
  'Etkileyici mimarisiyle {proje} projesinde {unit}',
  '{proje} projesinde etkileyici {unit}',
  '{proje} projesinde huzur ve konfor bir arada: {unit}',
  'Huzurun adresi {proje}: {unit}',
  '{proje} projesinde keyifli anlar: {unit}',
  'Keyifli bir yaşam sizi bekliyor — {proje} projesinde {unit}',
  '{proje} projesinde sıcacık bir yuva: {unit}',
  'Yeni yuvanız {proje} projesinde: {unit}',
  '{proje} projesinde ailenize özel {unit}',
  'Aile dostu {proje} projesinde {unit}',
  '{proje} projesinde çocuklu ailelere ideal {unit}',
  'İdeal konumda {unit} — {proje} projesinde',
  '{proje} projesinde merkezi konumda {unit}',
  'Ulaşımı kolay {proje} projesinde {unit}',
  '{proje} projesinde sosyal olanaklarıyla {unit}',
  'Zengin sosyal olanaklarıyla {proje} projesinde {unit}',
  '{proje} projesinde yaşamı kolaylaştıran {unit}',
  '{proje} projesinde konforlu yaşamın kapıları: {unit}',
  'Konforlu yaşam {proje} projesinde: {unit}',
  '{proje} projesinde prestijli bir yaşam: {unit}',
  'Prestijin adresi {proje}: {unit}',
  '{proje} projesinde seçkin bir çevrede {unit}',
  '{proje} projesinde ayrıcalıkların adresi: {unit}',
  '{proje} projesinde huzurlu komşuluk: {unit}',
  'Yatırımınıza güç katın — {proje} projesinde {unit}',
  '{proje} projesinde geleceğe sağlam adım: {unit}',
  'Sağlam bir yatırım {proje} projesinde: {unit}',
  '{proje} projesinde kaçırılmayacak yatırım: {unit}',
  '{proje} projesinde hayat kalitesini artıran {unit}',
  'Kaliteli yaşamın kapıları {proje} projesinde: {unit}',
  '{proje} projesinde titizlikle tasarlanmış {unit}',
  'Titizlikle işlenmiş {unit} — {proje} projesinde',
  '{proje} projesinde her mevsim keyifli {unit}',
  'Yeşille iç içe {proje} projesinde {unit}',
  'Doğayla bütünleşen {proje} projesinde {unit}',
  '{proje} projesinde doğanın içinde {unit}',
  'Sakin bir çevrede {unit} — {proje} projesinde',
  '{proje} projesinde sakin ve huzurlu {unit}',
  '{proje} projesinde yaşam kalitesi yüksek {unit}',
  'Yaşamın ayrıcalıklı yüzü {proje} projesinde: {unit}',
  '{proje} projesinde geleceğin yaşam alanı: {unit}',
];
...
const SEA_TEMPLATES = [
  ...
  '{proje} projesinde deniz manzarasına uyanın: {unit}',
  'Denizin sesiyle {proje} projesinde {unit}',
  '{proje} projesinde masmavi manzarasıyla {unit}',
  'Sahil yaşamının ayrıcalığı — {proje} projesinde {unit}',
  '{proje} projesinde deniz kokulu {unit}',
  'Deniz manzaralı huzur {proje} projesinde: {unit}',
  '{proje} projesinde denize karşı {unit}',
  'Enfes deniz manzarasıyla {proje} projesinde {unit}',
  '{proje} projesinde sahilin serinliğinde {unit}',
  'Deniz ve doğa bir arada — {proje} projesinde {unit}',
  '{proje} projesinde ufka kadar deniz: {unit}',
  'Kesintisiz deniz manzarasıyla {proje} projesinde {unit}',
  '{proje} projesinde denizle iç içe {unit}',
  'Mavinin huzuru {proje} projesinde: {unit}',
  '{proje} projesinde deniz esintisiyle serinleyen {unit}',
  'Sahil kasabası konforuyla {proje} projesinde {unit}',
];
...
function parseDaireText(raw) {
  ...
  const out = { proje: '', tip: '', oda: '', fiyat: null, cur: '', konut: '', m2: '', blok: '', daire_no: '' };
  ...
  // Oda (+, ', ’, &, -, /, x ayraçlarını kabul et — "4+2", "4'2", "2&1", "2-1"; ilk GEÇERLİ kombinasyon)
  for (const mm of t.matchAll(/(\d)\s*[+'’&\-\/xX]\s*(\d)/g)) { const o = mm[1] + '+' + mm[2]; if (ODA_TIPLERI.includes(o)) { out.oda = o; break; } }
  if (!out.oda && /st[üu]dyo/.test(t)) out.oda = 'Stüdyo';
  // Blok — önce "A blok", sonra "blok A"
  const bm = t.match(/\b([a-z]\d{0,2})\s*blok\b/) || t.match(/blok\s*:?\s*([a-z]\d{0,3}|\d{1,3}[a-z]?)/);
  if (bm) out.blok = bm[1].toUpperCase();
  // Daire no — "daire no 12" / "no 12" (arkasında oda gelmiyorsa) / "5 no"
  const dm = t.match(/(?:daire\s*no|d\.?\s*no|\bno)\s*:?\s*(\d{1,4})(?!\s*[+'’&\-\/xX]\s*\d)/) || t.match(/(\d{1,4})\s*(?:no\b|numara)/);
  if (dm) out.daire_no = dm[1];
  // Konut tipi
  ...
  // Fiyat — oda/m²/blok/daire desenlerini çıkar, kalan en büyük sayıyı (≥100) al. "100.000" = "100000".
  let s = t.replace(/\d\s*[+'’&\-\/xX]\s*\d/g, ' ').replace(/\d{2,4}\s*m[²2]/g, ' ')
    .replace(/\b[a-z]\d{0,2}\s*blok\b/g, ' ').replace(/blok\s*:?\s*[a-z]?\d{0,3}/g, ' ')
    .replace(/(?:daire\s*no|\bno)\s*:?\s*\d{1,4}/g, ' ').replace(/\d{1,4}\s*(?:no\b|numara)/g, ' ');
  let n = null; const km = s.match(/(\d{1,4})\s*(?:k|bin)\b/); if (km) n = parseInt(km[1], 10) * 1000;
  const nums = [...s.matchAll(/\d{1,3}(?:[.\s]\d{3})+|\d+/g)].map((m) => parseInt(m[0].replace(/[.\s]/g, ''), 10)).filter((x) => x >= 100);
  if (nums.length) n = Math.max(n || 0, ...nums);
  if (n) out.fiyat = n;
  // m²
...
function renderTaPreview() {
  ...
  $('#taPreview').innerHTML =
    ...
    (p.konut ? chip('Konut', p.konut, true) : '') +
    (p.blok ? chip('Blok', p.blok, true) : '') + (p.daire_no ? chip('Daire No', p.daire_no, true) : '') +
    (p.m2 ? chip('m²', p.m2, true) : '');
}
// Çözümlenen alanları (gizli) forma yerleştir — proje seçilmiş gibi otomatik dolum
function setParsedFields(p) {
  ...
  $('#f_cur').value = p.cur || 'GBP';
  if (p.fiyat != null) $('#f_fiyat').value = Number(p.fiyat).toLocaleString('tr-TR');
  if (p.m2) $('#f_m2').value = p.m2;
  if (p.blok) $('#f_blok').value = p.blok;
  if (p.daire_no) $('#f_daire_no').value = p.daire_no;
  if (p.proje) $('#f_proje').dispatchEvent(new Event('change'));   // il/ilçe + olanaklar + ortak fotoğraflar + başlık
  else applyAutoTitle();
  updateKatVisibility();
}
...
$('#taGo').addEventListener('click', () => {
  ...
  openProp(null);          // durumu sıfırla (editId=null, photos=[], varsayılan danışman)
  setParsedFields(parsed);
  const hasRegion = !!($('#f_il').value || $('#f_bolge').value);
  closeModal($('#textAddModal'));
  if (hasRegion) {
    // Yazıldığı gibi DİREKT ekle — form penceresi açılmaz
    closeModal($('#propModal'));
    $('#savePropBtn').click();
  } else {
    // Proje/bölge çıkmadıysa formu açık bırak, bölgeyi seçip kaydetsin
    toast('Bölge belirlenemedi — bölgeyi seçip kaydet', 'err');
  }
});
